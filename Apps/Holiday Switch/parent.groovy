...
definition(
    ...
    description: "This is the 'Parent' app for Holiday Switch",
...
def mainPage() {
	...
	display()
	section (){app(name: "newApp", appName: "Holiday Switch Child", namespace: "Cobra", title: "<b>Schedule a new Holiday event</b>", multiple: true)}
	displayDisable()
	}
  }
}
...
def version(){
    ...
    schedule("0 0 " + randomHour + " ? * " + randomDayOfWeek, updateCheck)   //  Check for updates
    updateCheck()  
    checkButtons()
...
def setVersion(){
     state.version = "1.0.0"
     state.InternalName = "HolidaySwitchParent"
	 state.CobraAppCheck = "holidayswitch.json"
	 state.ExternalName = "Holiday Switch Parent"
