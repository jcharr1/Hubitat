...
def setInternal(){
	logger("info", "Humidity and Temperature set to use internal values")
	state.changeOver = true
}
def setExternal(){
	logger("info", "Humidity and Temperature set to use external values")
	state.changeOver = false
}	
...
def setWeather(weather){
	...
	if(state.changeOver == true){sendEvent(name: "temperature", value: weather.tempinf, unit: '°F', isStateChange: true)}
	if(state.changeOver == false){sendEvent(name: "temperature", value: weather.tempf, unit: '°F', isStateChange: true)}
	
	//Set Indoor temperature
	sendEvent(name: "temperaturein", value: weather.tempinf, unit: '°F', isStateChange: true);

	//Set Humidity
	if(state.changeOver == true){sendEvent(name: "humidity", value: weather.humidityin, unit: '%', isStateChange: true)}
	if(state.changeOver == false){sendEvent(name: "humidity", value: weather.humidity, unit: '%', isStateChange: true)}
...
// internal/external temp & humidity settings added my @Cobra